...
public abstract class HandyHaversacksBase implements Puzzle<String[]> {
  ...
  protected final Map<String, Bag> INPUT_PROCESSED;

  public static void main(String[] args) {
    new HandyHaversacksTwo().resolve();
  }

  protected HandyHaversacksBase() {
    this.INPUT_PROCESSED = new HashMap<>();
    for (String bag : INPUT) {
      String[] split = bag.split(" bags contain ");
      String name = split[0];
      String properties = split[1];
      INPUT_PROCESSED.put(name, new Bag(name, properties));
    }
  }

  protected int deepCountOfBags(String bagTarget, Bag bag) {
    int count = 0;
    for (Map.Entry<String, Integer> entry : bag.getBags().entrySet()) {
      String bagName = entry.getKey();
      if (bagName.equals(bagTarget)) {
        count += entry.getValue();
        continue;
      }
      Bag bagEntry = INPUT_PROCESSED.get(bagName);
      count += deepCountOfBags(bagTarget, bagEntry);
    }
    return count;
  }


  public long deepCountAllBags(Bag bag) {
    return deepCountAllBags(bag, INPUT_PROCESSED);
  }

  public static long deepCountAllBags(Bag bag, Map<String, Bag> bagMap) {
    long count = 1;
    Map<String, Integer> subBags = bag.getBags();
    if (subBags.isEmpty()) return count;
    for (Map.Entry<String, Integer> entry : subBags.entrySet()) {
      String subBagName = entry.getKey();
      Integer multiplier = entry.getValue();
      Bag subBag = bagMap.get(subBagName);
      count += deepCountAllBags(subBag, bagMap) * multiplier;
    }
    return count;
  }
